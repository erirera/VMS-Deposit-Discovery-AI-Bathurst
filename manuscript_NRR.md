...
Random Forest (RF) and Extreme Gradient Boosting (XGBoost) classifiers were trained on 295 spatial labels comprising 45 known VMS deposits and 250 hybrid negative labels—125 confirmed barren drill intercepts from New Brunswick Geological Survey records and 125 feature-space dissimilar pseudo-absences selected by maximising Mahalanobis distance from the VMS deposit centroid in multi-dimensional geophysical and geochemical feature space (Parsa & Cumani, 2025). Synthetic Minority Over-sampling Technique (SMOTE) was used for class balancing, and 5-fold spatial block cross-validation was applied to address spatial autocorrelation. Model performance was evaluated using Receiver Operating Characteristic (ROC), Precision-Recall (PR), cumulative Success Rate, and Balanced Accuracy (BA) metrics. RF outperformed XGBoost across all four metrics: ROC-AUC 0.910 ± 0.060 vs. 0.896 ± 0.073; Average Precision 0.688 ± 0.128 vs. 0.589 ± 0.184; Balanced Accuracy 0.757 ± 0.103 vs. 0.796 ± 0.120; and Success Rate AUC 0.903 vs. 0.889. The RF model captures 97.5% of known BMC VMS deposits within the top 30% of the study area ranked by prospectivity index. The magnetic Tilt Derivative emerged as the dominant predictor in both classifiers, demonstrating the value of integrating structural geophysical derivatives with multi-element till geochemistry for covered VMS exploration.
...
|---|---|---|
| ROC-AUC (mean ± SD) | **0.910 ± 0.060** | 0.896 ± 0.073 |
| Average Precision (mean ± SD) | **0.688 ± 0.128** | 0.589 ± 0.184 |
| Balanced Accuracy (mean ± SD) | 0.757 ± 0.103 | **0.796 ± 0.120** |
| Success Rate AUC | **0.903** | 0.889 |

RF outperformed XGBoost on ROC-AUC (0.910 ± 0.060 vs. 0.896 ± 0.073), Average Precision (0.688 ± 0.128 vs. 0.589 ± 0.184), and Success Rate AUC (0.903 vs. 0.889). XGBoost achieved a marginally higher Balanced Accuracy (0.796 ± 0.120 vs. 0.757 ± 0.103), reflecting somewhat more balanced sensitivity-specificity trade-off at the default 0.5 threshold. Fold-to-fold standard deviations were substantially reduced compared to previous model iterations (RF ROC-AUC SD: ±0.060; XGBoost: ±0.073), indicating improved generalisation stability across geographic blocks.
...
RF outperformed XGBoost on ROC-AUC, Average Precision, and Success Rate AUC. The highly concentrated feature dependence observed in XGBoost (TDR: 42% of total SHAP mass) indicates that it captured the dominant structural signal effectively but underutilised the geochemical predictors. RF's ensemble averaging across independently grown trees distributes predictive weight more evenly across features, which is advantageous when prospectivity is determined by the intersection of structural, geochemical, and radiometric criteria simultaneously (Breiman, 2001; Harris et al., 2015).

XGBoost achieved a marginally higher Balanced Accuracy (0.796 ± 0.120 vs. 0.757 ± 0.103 for RF), suggesting a more balanced sensitivity-specificity trade-off at the default decision threshold. Both models confirmed that SMOTE augmentation did not introduce systematic minority-class over-prediction bias. The substantially reduced fold-to-fold variability relative to previous model iterations (RF ROC-AUC SD: ±0.060; XGBoost: ±0.073) indicates improved generalisation stability, likely reflecting the cleaner negative label set generated by Mahalanobis-distance selection. Translated to exploration practice, the RF model captured 65% of known BMC VMS deposits within the top 10% and 97.5% within the top 30% of the study area ranked by prospectivity index — a SR-AUC of 0.903, substantially above random expectation (0.5) and comparable to state-of-the-art MPM benchmarks reported for analogous camp-scale datasets.
...
Computing geophysical derivatives in the Fourier domain directly on the original survey grids, prior to spatial resampling to 100 m, preserved high-frequency structural and lithological boundary information that resampling-induced edge effects would otherwise degrade. The magnetic Tilt Derivative and Total Horizontal Gradient were the dominant predictors in both classifiers, confirming that structural edge detection captures the primary spatial control on VMS prospectivity in the BMC — specifically the northeast-trending shear zones and volcanic contact systems of the Tetagouche Group.

Centered Log-Ratio transformation of the till geochemistry dataset, followed by PCA and FA with varimax rotation, resolved four geochemical associations corresponding to distinct hydrothermal and lithogeochemical signatures. The CLR-FA Factor 4 composite — characterised by an Ag–Mo depletion / Bi–Cd enrichment signature — ranked as the highest-importance geochemical predictor in both models, demonstrating that CoDA-compliant multivariate decomposition resolves deposit-scale hydrothermal footprints obscured in single-element analysis. Retaining raw elemental concentrations (Pb, Mo, Fe) as independent point-scale features alongside the CLR-FA composites further improved model performance by capturing short-range geochemical contrasts suppressed by IDW spatial smoothing.

A hybrid negative label strategy combining 125 confirmed geologically barren drill intercepts with 125 feature-space dissimilar pseudo-absences — selected by maximising Mahalanobis distance from the VMS deposit centroid in multi-dimensional feature space (Parsa & Cumani, 2025) — produced a more defensible training dataset than purely random or distance-filtered pseudo-absences. This approach ensures that pseudo-negatives represent geological environments maximally unlike those of known VMS deposits, rather than simply distant in map space. Under 5-fold spatial block cross-validation, the Random Forest classifier outperformed XGBoost on ROC-AUC (0.910 ± 0.060 vs. 0.896 ± 0.073), Average Precision (0.688 ± 0.128 vs. 0.589 ± 0.184), and Success Rate AUC (0.903 vs. 0.889), capturing 97.5% of known BMC VMS deposits within the top 30% of the study area ranked by prospectivity index. XGBoost achieved marginally higher Balanced Accuracy (0.796 ± 0.120 vs. 0.757 ± 0.103). The substantially reduced fold-to-fold variability confirms that the Mahalanobis-guided negative label strategy improved cross-validation stability relative to distance-filtered or random pseudo-absence approaches.
...
The small positive training set (n = 45) contributes to fold-to-fold cross-validation variability and remains the principal limitation of the study. Future work should incorporate additional VMS occurrences from adjacent districts and integrate depth-sensitive geophysical methods — including induced polarization, time-domain electromagnetics, and 3D seismic reflection — to extend the framework into three dimensions. The pipeline is directly transferable to other glacially covered VMS districts, including the Abitibi Greenstone Belt and the Flin Flon–Snow Lake camp, where analogous challenges of structural complexity and overburden geochemical dilution present equivalent exploration problems.
...
**Fig. 6** Spatial distribution of training labels overlaid on the total magnetic intensity (TMI) background grid of the BMC. Positive labels (VMS deposits, n = 45) shown as yellow stars; confirmed barren drill holes (n = 125) shown as red triangles; feature-space dissimilar pseudo-absences (n = 125) shown as open circles, coloured by Mahalanobis distance from the VMS deposit centroid (darker = more dissimilar). Pseudo-absences are distributed across all four geographic quadrants of the survey footprint via stratified spatial sampling. A secondary 1,000 m geographic guard buffer around known deposits is indicated by the dashed boundary. Projection: EPSG:2953
