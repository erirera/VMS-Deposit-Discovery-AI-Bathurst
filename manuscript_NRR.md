...
Random Forest (RF) and Extreme Gradient Boosting (XGBoost) classifiers were trained on 295 spatial labels comprising 45 known VMS deposits and 250 hybrid negative labels—125 confirmed barren drill intercepts from New Brunswick Geological Survey records sampled strictly within the master BMC study area raster grid and 125 feature-space dissimilar negative labels selected by maximising Mahalanobis distance from the VMS deposit centroid in multi-dimensional feature space. All 17 raw geochemical elements were retained for model training; the four elements with the highest sparse-data rates (Bi 60.3%, In 60.3%, Tl 60.3%, Mn 58.3%) were included as sparse features alongside their spatially complete IDW-interpolated counterparts, with remaining missing values imputed by column-wise median. Synthetic Minority Over-sampling Technique (SMOTE) was used for class balancing, and 5-fold spatial block cross-validation was applied to address spatial autocorrelation. Model performance was evaluated using Receiver Operating Characteristic (ROC), Precision-Recall (PR), cumulative Success Rate, and Balanced Accuracy (BA) metrics. RF outperformed XGBoost across key discrimination and targeting metrics: ROC-AUC 0.9318 ± 0.0368 vs. 0.9098 ± 0.0369; Average Precision 0.7245 ± 0.1476 vs. 0.6226 ± 0.1481; Balanced Accuracy 0.8261 ± 0.0701 vs. 0.8456 ± 0.0654; and Success Rate AUC 0.9680 vs. 0.9494. The RF model captures 91.1% of known BMC VMS deposits within the top 10%, 97.8% within the top 20%, and 100.0% within the top 30% of the study area ranked by prospectivity index; XGBoost captures 80.0% within the top 10%, 97.8% within the top 20%, and 100.0% within the top 30%. The radiometric Th/K alteration ratio emerged as the dominant predictor in both classifiers, followed by molybdenum, zinc, and lead till geochemistry pathfinders and gravity horizontal gradients, demonstrating the value of integrating hydrothermal alteration radiometrics with structural geophysical derivatives and multi-element till geochemistry for covered VMS exploration.
...
subgraph P4["Phase 4 · Spatial Block Cross-Validation & Model Training"]
    ...
    RF: ROC-AUC 0.9318 ± 0.0368 · SR-AUC 0.9680"]
end
...
Features containing more than 75% missing values were excluded from model training to prevent numerical instability. All 17 raw geochemical elements fell below this threshold and were retained. The four elements with the highest sparse-data rates—Bi (60.3%), In (60.3%), Tl (60.3%), and Mn (58.3%)—were included as sparse features alongside their spatially complete IDW-interpolated counterparts, which were available for all 295 sample locations. Missing values in retained sparse features were imputed using column-wise median values computed from the training fold only, ensuring no data leakage into validation folds.
...
RF and XGBoost classifiers were trained and hyperparameter-tuned using Optuna-based randomized search (Akiba et al., 2019) over the spatial cross-validation folds. RF was configured with Gini impurity split criteria (Breiman, 2001), and XGBoost was trained using a binary logistic objective function (Chen and Guestrin, 2016). Class weights were set to 'balanced' for both classifiers to further mitigate residual class imbalance effects.
...
Individual single-element geochemistry databases for 17 elements (Ag, As, Ba, Bi, Cd, Co, Cu, Fe, In, Mn, Mo, Ni, Pb, Sb, Sn, Tl, Zn) were merged by coordinate matching into a unified spatial database of **2,753 unique sample locations**. IDW interpolation produced 17 element-specific raster surfaces at 50 m resolution (Fig. 3g–i). All 17 raw elements were retained as geochemical predictors; no element exceeded the 75% null threshold applied during feature selection. The four elements with the highest sparse-data rates—Bi (60.3%), In (60.3%), Tl (60.3%), and Mn (58.3%)—were retained as sparse point-scale features alongside their spatially complete IDW-interpolated counterparts, which were populated for all 295 label locations. Remaining missing values in sparse features were imputed with column-wise median values computed within each training fold.
...
CLR-FA with varimax rotation yielded four factors. FA Factor 2 (`geochem_fa_factor2_idw`) — the highest-ranked geochemical composite predictor in the RF model (mean |SHAP| = 0.0126; Section 4.4.1) — was characterised by strong negative loadings for Cd (−0.625) and Bi (−0.603), and strong positive loadings for Co (0.664) and Cu (0.641), reflecting a base-metal hydrothermal co-enrichment signature associated with proximal VMS stockwork mineralization. FA Factor 1 showed broad positive loadings for Zn (0.873), Pb (0.843), Mn (0.923), Ni (0.776), Ba (0.779), Tl (0.777), Sb (0.728), Co (0.712), and Cu (0.642), capturing the polymetallic dispersion halo of BMC VMS systems. FA Factor 4 was characterised by negative loadings for Ag (−0.362) and Mo (−0.449) and a positive loading for Bi (0.513). Spatial raster surfaces of all four CLR-PCA and CLR-FA scores are shown in Fig. 5.
...
RF outperformed XGBoost on ROC-AUC (0.9318 ± 0.0368 vs. 0.9098 ± 0.0369), Average Precision (0.7245 ± 0.1476 vs. 0.6226 ± 0.1481), and Success Rate AUC (0.9680 vs. 0.9494). XGBoost achieved a marginally higher Balanced Accuracy (0.8456 ± 0.0654 vs. 0.8261 ± 0.0701), reflecting a slightly higher true negative classification rate at the default 0.5 decision threshold. Standard deviations across spatial folds were low (RF ROC-AUC SD: ±0.0368; XGBoost SD: ±0.0369), demonstrating robust generalisation stability across geographically disjoint blocks of the Bathurst Mining Camp.

> *[Figure 8 near here]*

### 4.4 Feature Importance and SHAP Analysis

SHAP mean absolute values were computed for all predictor features across the RF and XGBoost models (Fig. 10). Results are reported for the top 10 features by mean |SHAP| for each model. Note that RF SHAP values are expressed in probability units while XGBoost SHAP values are in log-odds units; the two scales are not numerically comparable, and cross-model comparisons should focus on feature rank rather than absolute magnitude.

#### 4.4.1 Random Forest Feature Importance

The radiometric Th/K ratio (`rad_th_k_bmc`) emerged as the top predictor for RF (mean |SHAP| = 0.0531), directly mapping potassic and sericitic hydrothermal alteration halos associated with BMC VMS feeder systems. Raw thorium (`rad_th_bmc`; 0.0435) ranked second, consistent with Th depletion in hydrothermally altered wall-rocks. IDW-interpolated molybdenum (`geochem_mo_ppm_idw`; 0.0340) ranked third, reflecting the association of Mo with high-temperature hydrothermal fluids and VMS feeder pipes. The remaining top-10 RF features in descending order were: airborne potassium (`rad_k_bmc`; 0.0232), IDW zinc (`geochem_zn_ppm_idw`; 0.0208), IDW bismuth (`geochem_bi_ppm_idw`; 0.0170), gravity horizontal gradient magnitude (`gra_ggr_hgm_bmc`; 0.0166), IDW lead (`geochem_pb_ppm_idw`; 0.0161), raw molybdenum (`mo_ppm`; 0.0128), and CLR-FA Factor 2 (`geochem_fa_factor2_idw`; 0.0126).

#### 4.4.2 XGBoost Feature Importance

The XGBoost model (mean |SHAP| values reported in log-odds units) similarly identified the radiometric Th/K ratio (`rad_th_k_bmc`; 1.1991) as the strongly dominant predictor, with attribution substantially exceeding all other features. IDW-interpolated molybdenum (`geochem_mo_ppm_idw`; 0.5331) ranked second, followed by raw zinc (`zn_ppm`; 0.4783), IDW tin (`geochem_sn_ppm_idw`; 0.4046), and upward-continued Bouguer gravity (`gra_ggr_uc500_bmc`; 0.3952), which captures long-wavelength crustal density contrasts associated with the volcanic-sedimentary pile hosting BMC mineralization. Structural predictors featured prominently: radiometric U/Th ratio (`rad_u_th_bmc`; 0.3735), raw molybdenum (`mo_ppm`; 0.3656), raw nickel (`ni_ppm`; 0.3580), magnetic analytic signal amplitude (`mag_rmi_as_bmc`; 0.3483), and magnetic first vertical derivative (`mag_rmi_fvd_bmc`; 0.3261) completed the top-10 features.

Across both models, key predictor domains consistently dominated: (1) radiometric radioelement ratios mapping hydrothermal alteration halos (Th/K, Th, K, U/Th); (2) till geochemistry pathfinder anomalies (Mo, Zn, Pb, Sn, Ni); and (3) potential-field gravity and magnetic structural derivatives (gravity upward continuation, magnetic analytic signal, FVD) delineating syn-volcanic fault zones.

> *[Figure 10 near here]*

### 4.5 Prospectivity Map

The RF model was applied to the full 100 m prediction grid (953 × 1,253 = 1,194,109 cells), generating the BMC VMS prospectivity map (Fig. 9). The Prospectivity Index (PI) ranged from 0 to 1 with a median PI of 0.049. High-priority exploration targets (PI > 0.7) spanned 23,585 cells, representing just **2.0%** of the BMC study area, while moderate-to-high prospectivity (PI > 0.5) covered 85,303 cells (**7.1%** of the study area); very high-priority zones (PI > 0.9) comprised 2,678 cells (0.2% of the study area). High-prospectivity zones formed spatially coherent, elongate anomalies trending northeast along the Tetagouche Group volcanic horizons. Several high-PI anomalies (PI > 0.7) occurred in areas with no currently known VMS deposits, presenting first-pass exploration targets in covered terrain. The prospectivity map was exported as a georeferenced GeoTIFF (EPSG:2953) for GIS-based drill targeting workflows.
...
The radiometric Th/K ratio (`rad_th_k_bmc`) emerged as the strongest single predictor in both classifiers, confirming that hydrothermal alteration signatures — specifically the potassic enrichment and thorium depletion characterising sericitic VMS feeder halos — exert the primary spatial control on prospectivity in the BMC (Shives et al., 1997). Structural geophysical derivatives played a supporting but significant role: the gravity horizontal gradient magnitude (`gra_ggr_hgm_bmc`) ranked seventh for RF, while the magnetic analytic signal and first vertical derivative ranked ninth and tenth for XGBoost, collectively delineating the northeast-trending syn-volcanic fault zones and volcanic contacts of the Tetagouche Group that channelled hydrothermal fluids (Van Staal et al., 2003; Thomas et al., 2000). Computing derivatives in the Fourier domain directly on the original survey grids preserved the high-frequency structural signals that would be attenuated by resampling prior to differentiation (Blakely, 1995), which is critical in the BMC where deposit-scale structural contrasts occur over distances comparable to the original survey line spacing.
...
IDW-interpolated molybdenum (`geochem_mo_ppm_idw`) was the highest-ranked individual geochemical predictor in both RF (mean |SHAP| = 0.0340, rank 3) and XGBoost (0.5331 log-odds units, rank 2), consistent with Mo enrichment in high-temperature hydrothermal fluids associated with VMS feeder systems and stockwork zones (Franklin et al., 2005). Raw zinc (`zn_ppm`) and IDW tin (`geochem_sn_ppm_idw`) featured prominently in XGBoost (ranks 3 and 4 respectively), reflecting the polymetallic Zn–Pb–Cu–Sn character of BMC VMS deposits. CLR-FA Factor 2 (`geochem_fa_factor2_idw`) appeared in the RF top-10 (rank 10; mean |SHAP| = 0.0126), capturing the Co–Cu enrichment versus Bi–Cd depletion contrast characteristic of proximal VMS stockwork zones. The parallel ranking of raw pathfinder concentrations (Zn, Mo, Ni) alongside IDW-smoothed geochemical surfaces confirms that point-scale and regional-scale representations provide complementary predictive information — IDW surfaces capture camp-scale dispersion halos while raw concentrations preserve deposit-proximal high-contrast anomalies suppressed by spatial smoothing — consistent with multi-scale integration recommendations for data-driven MPM (Parsa, 2022).

### 5.3 Classifier Performance and Algorithm Comparison

RF outperformed XGBoost on ROC-AUC (0.9318 ± 0.0368 vs. 0.9098 ± 0.0369), Average Precision (0.7245 ± 0.1476 vs. 0.6226 ± 0.1481), and Success Rate AUC (0.9680 vs. 0.9494). XGBoost achieved a marginally higher Balanced Accuracy (0.8456 ± 0.0654 vs. 0.8261 ± 0.0701), reflecting a slightly higher true negative classification rate at the default 0.5 decision threshold. Both models confirmed that SMOTE augmentation did not introduce systematic minority-class over-prediction bias. The low fold-to-fold variability (RF ROC-AUC SD: ±0.0368; XGBoost SD: ±0.0369) indicates exceptional generalisation stability, reflecting the clean negative label set generated by Mahalanobis-distance feature dissimilarity selection strictly within the master BMC study area bounds. Selecting feature-space dissimilar negative labels via covariance-weighted Mahalanobis distance prevented correlated geochemical pathfinders and potential-field derivatives from distorting feature dissimilarity, delivering a well-separated negative training pool that enhanced fold-to-fold classification stability. Translated to exploration practice, the RF model captured **91.1%** of known BMC VMS deposits within the top **10%**, **97.8%** within the top **20%**, and **100.0%** within the top **30%** of the study area ranked by prospectivity index (SR-AUC = **0.9680**); XGBoost captured **80.0%** within the top **10%**, **97.8%** within the top **20%**, and **100.0%** within the top **30%** (SR-AUC = **0.9494**). Both models substantially outperform random expectation (SR-AUC = 0.5), confirming the targeting efficiency of the prospectivity framework across both classifiers.
...
Computing geophysical derivatives in the Fourier domain directly on the original survey grids, prior to spatial resampling to 100 m, preserved high-frequency structural and lithological boundary information that resampling-induced edge effects would otherwise degrade. The radiometric Th/K alteration ratio, the dominant predictor in both classifiers, directly maps the potassic and sericitic hydrothermal halos of VMS feeder systems. Structural potential-field derivatives (gravity horizontal gradient, magnetic analytic signal and first vertical derivative) alongside till geochemistry pathfinders (Mo, Zn, Pb) captured complementary spatial controls on VMS prospectivity in the BMC.
...
A hybrid negative label strategy combining 125 confirmed geologically barren drill intercepts sampled strictly within master BMC raster bounds with 125 feature-space dissimilar negative labels — selected by maximising Mahalanobis distance from the VMS deposit centroid in multi-dimensional feature space (Parsa & Cumani, 2025) — produced a highly defensible training dataset. Under 5-fold spatial block cross-validation, the Random Forest classifier outperformed XGBoost on ROC-AUC (**0.9318 ± 0.0368** vs. 0.9098 ± 0.0369), Average Precision (**0.7245 ± 0.1476** vs. 0.6226 ± 0.1481), and Success Rate AUC (**0.9680** vs. 0.9494). RF captured **91.1%** of known BMC VMS deposits within the top 10%, **97.8%** within the top 20%, and **100.0%** within the top 30% of the study area; XGBoost captured **80.0%** within the top 10%, **97.8%** within the top 20%, and **100.0%** within the top 30%. Both models substantially outperform random expectation (SR-AUC = 0.5), confirming the targeting efficiency of the prospectivity framework.
...
**Fig. 8** Model evaluation curves for Random Forest (RF) and XGBoost (XGB) classifiers under 5-fold spatial block cross-validation. (a) Receiver Operating Characteristic (ROC) curves; mean ROC-AUC: RF = 0.9318 ± 0.0368, XGB = 0.9098 ± 0.0369. (b) Precision-Recall (PR) curves; mean Average Precision: RF = 0.7245 ± 0.1476, XGB = 0.6226 ± 0.1481. (c) Cumulative Success Rate (Prediction-Area) curves plotting the percentage of known VMS deposits captured against the percentage of total study area covered when cells are ranked by prospectivity probability in descending order; Success Rate AUC: RF = 0.9680, XGB = 0.9494. Shaded bands indicate ± 1 standard deviation across folds. The diagonal dashed line represents random prediction

**Fig. 9** Random Forest prospectivity map of the Bathurst Mining Camp at 100 m spatial resolution. Prospectivity index (PI) ranges from 0 (low probability of VMS mineralization) to 1 (high probability). Known VMS deposits (yellow stars) and confirmed barren drill holes (red triangles) used in model training are overlaid for validation context. High-priority exploration target zones (PI > 0.7; 2.0% of study area) are outlined with black contours; moderate-to-high prospectivity zones (PI > 0.5; 7.1% of study area) are shown in warm tones; very high-priority zones (PI > 0.9; 0.2% of study area) represent the most focused drill targets. Geological contacts (thin grey lines) are shown for reference. Projection: EPSG:2953; exported as GeoTIFF for GIS integration
