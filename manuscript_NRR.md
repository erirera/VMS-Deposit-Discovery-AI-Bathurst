...
IDW-interpolated molybdenum (`geochem_mo_ppm_idw`) was the highest-ranked geochemical composite predictor in both RF (mean |SHAP| = 0.0340) and XGBoost (1.1991 log-odds units), consistent with Mo enrichment in high-temperature hydrothermal fluids associated with VMS feeder systems and stockwork zones (Franklin et al., 2005). Raw zinc (`zn_ppm`) and IDW tin (`geochem_sn_ppm_idw`) featured prominently in XGBoost, reflecting the polymetallic Zn–Pb–Cu–Sn character of BMC VMS deposits. CLR-FA Factor 2 appeared in the RF top-10 (mean |SHAP| = 0.0126), linking spatially to the broad base-metal hydrothermal dispersion halo. The parallel ranking of raw pathfinder concentrations (Zn, Mo, Ni) alongside IDW-smoothed geochemical surfaces confirms that point-scale and regional-scale representations provide complementary predictive information — IDW surfaces capture camp-scale dispersion halos while raw concentrations preserve deposit-proximal high-contrast anomalies suppressed by spatial smoothing — consistent with multi-scale integration recommendations for data-driven MPM (Parsa, 2022).
