...
The final training dataset comprised 295 spatially distributed labels: 45 known VMS deposits (positive class, $Y = 1$) and 250 hybrid negative labels ($Y = 0$) consisting of 125 confirmed barren drill holes and 125 spatially constrained pseudo-absences (Fig. 6). Following SMOTE augmentation of the minority class, the balanced training set contained $n = 500$ samples (250 per class).
...
A hybrid negative label strategy combining 125 confirmed geologically barren drill intercepts with 125 spatially constrained pseudo-absences produced a more defensible training dataset than purely random pseudo-absences. Under 5-fold spatial block cross-validation, the Random Forest classifier outperformed XGBoost across all four evaluation metrics — ROC-AUC, Average Precision, Balanced Accuracy, and Success Rate AUC — capturing approximately 80% of known BMC VMS deposits within the top 30% of the study area ranked by prospectivity index. The RF Balanced Accuracy of 0.697 confirmed that SMOTE augmentation did not introduce minority-class over-prediction bias.
...
**Fig. 6** Spatial distribution of training labels overlaid on the total magnetic intensity (TMI) background grid of the BMC. Positive labels (VMS deposits, n = 45) shown as yellow stars; confirmed barren drill holes (n = 125) shown as red triangles; spatially constrained pseudo-absences (n = 125) shown as open circles. The 3,000 m exclusion buffer applied around known VMS deposits during pseudo-absence generation is indicated by the dashed boundary. Projection: EPSG:2953
