"""
mean_shap_rf_probability.py
───────────────────────────
Compute Mean SHAP values for Random Forest in probability space.

This script:
  1. Loads the trained Random Forest model and training data
  2. Computes SHAP values in probability space
...
Usage:
    python pipeline/05_explainability/mean_shap_rf_probability.py
"""
...
from config import (
    PROCESSED_DIR, MODELS_DIR, OUTPUTS_DIR, RF_MODEL_PATH
)
...
def compute_mean_shap_rf_probability(model, X: np.ndarray, feature_names: list):
    """
    Compute SHAP values for Random Forest in probability space.
    
    Approach:
      1. Create background dataset from training data
...
def save_mean_shap_table(mean_shap_df, model_name="RandomForest"):
...
def plot_mean_shap_bar(mean_shap_df, model_name="RandomForest", top_n=15):
    ...
    with plt.rc_context(STYLE):
        ...
        bars = ax.barh(
            ...
            color="#f59e0b", alpha=0.85, edgecolor="#d97706"
        )
...
def plot_mean_shap_stats(mean_shap_df, y_pred_prob, model_name="RandomForest"):
    """Create statistical summary plot."""
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    
    # Left: Top 10 features
    top_10 = mean_shap_df.head(10)
    axes[0].barh(top_10['feature'], top_10['mean_shap_abs'], color="#f59e0b", alpha=0.85)
    ...
    axes[1].hist(y_pred_prob, bins=40, color="#ec4899", alpha=0.7, edgecolor="#be185d")
    ...
    fig.suptitle(
        f"Random Forest Mean SHAP Summary (Probability Space)",
...
def generate_summary_report(mean_shap_df, base_prob, model_name="RandomForest"):
...
def main():
    log.info("═══ Mean SHAP Analysis for Random Forest (Probability Space) ═══")
    log.info("")
    ...
    results = compute_mean_shap_rf_probability(model, X, feature_names)
    ...
    # Save results
    log.info("\n─── Saving Results ───")
    save_mean_shap_table(mean_shap_df, model_name="RandomForest")
    
    # Generate visualizations
    log.info("\n─── Generating Visualizations ───")
    plot_mean_shap_bar(mean_shap_df, model_name="RandomForest", top_n=15)
    plot_mean_shap_stats(mean_shap_df, y_pred_prob, model_name="RandomForest")

    # Print summary
    log.info("\n─── Summary ───")
    summary = generate_summary_report(mean_shap_df, base_prob, model_name="RandomForest")
    log.info(summary)
    
    # Save summary report
    report_out = SHAP_DIR / "mean_shap_rf_probability_report.txt"
